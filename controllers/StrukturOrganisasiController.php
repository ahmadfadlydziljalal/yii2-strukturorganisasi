...
use ahmadfadlydziljalal\strukturorganisasi\models\search\StrukturOrganisasiSearch;
...
class StrukturOrganisasiController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            Yii::$app->session->setFlash(
                ...
                Struktur Organisasi : " . $model->nama . " berhasil ditambahkan. " . Html::a('Klik link berikut jika ingin melihat detailnya', ['view', 'id' => $model->id], ['class' => 'btn btn-link'])
            );
    ...
    public function actionUpdate($id, $page = null)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            Yii::$app->session->setFlash(
                'info',
                FAS::icon(FAS::_THUMBS_UP) . "
                Struktur Organisasi : " . $model->nama . " berhasil di update. " . Html::a('Klik link berikut jika ingin melihat detailnya', ['view', 'id' => $model->id], ['class' => 'btn btn-link'])
            );
    ...
    public function actionDelete($id, $page = null)
    {
        ...
        Yii::$app->session->setFlash('danger', FAS::icon(FAS::_SAD_CRY) . " Struktur Organisasi : " . $oldLabel . " successfully deleted.");
        return $this->redirect(['index', 'page' => $page]);
    }
    ...
    public function actionIndexDiagram($parent_id)
    {
        ...
        $nodes = Tree::buildTree(
            StrukturOrganisasi::generateTree($parent_id),
            $parent->parent_id
        );
    ...
    public function actionUpdateChildFromDiagram($root_id, $parent_id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            ...
            Yii::$app->session->setFlash(
                'success',
                FAS::icon(FAS::_THUMBS_UP) . "
                Struktur Organisasi : " . $model->nama . " berhasil diupdate. " . Html::a('Klik link berikut jika ingin melihat detailnya', ['view', 'id' => $model->id], ['class' => 'btn btn-link'])
            );
